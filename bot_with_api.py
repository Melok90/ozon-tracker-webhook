...
# Хранилище для мониторингов пользователей
monitors = {}
...
def get_monitors():
    """
    Получить список всех мониторингов
    ...
    user_monitors = monitors.get(int(user_id), {})
...
def get_price(url):
    """Получает текущую цену товара с Ozon"""
    headers = {
    ...
    try:
        ...
        # Попытка найти цену (паттерны могут меняться)
        price_patterns = [
        ...
        for pattern in price_patterns:
            price_element = soup.find("span", pattern)
            if price_element:
        ...
        print(f"[WARN] Не удалось найти цену на странице")
        return None
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_price is None:
        await update.message.reply_text(
            ...
            f"🔗 Товар: {url}\n\n"
            f"Буду проверять каждые 2 часа!"
        )
    else:
        if current_price <= target_price:
            await update.message.reply_text(
                f"🎉 Отлично! Цена уже ниже целевой!\n\n"
            ...
            await update.message.reply_text(
                ...
                f"🎯 Целевая цена: {target_price} ₽\n"
                f"🔗 Товар: {url}\n\n"
                f"Я уведомлю тебя, когда цена снизится!"
            )
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_price is None:
        await update.message.reply_text("⚠️ Не удалось получить текущую цену.\nПопробую позже!")
    else:
        if current_price <= monitor["target_price"]:
            await update.message.reply_text(
                f"🎉 Цена достигла цели!\n\n"
                f"💰 Текущая цена: {current_price} ₽\n"
...
async def check_prices_job(context: ContextTypes.DEFAULT_TYPE):
    """Периодическая проверка цен для всех пользователей"""
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Проверка цен...")
    
    for chat_id, monitor in list(monitors.items()):
        try:
            current_price = get_price(monitor["url"])
            ...
            print(f"Ошибка при проверке для {chat_id}: {e}")
